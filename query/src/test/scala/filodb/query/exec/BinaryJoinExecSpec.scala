...
class BinaryJoinExecSpec extends AnyFunSpec with Matchers with ScalaFutures {
  ...
  it("should deal with implictly added step and pi tag as join key on OneToMany joins") {
    ...
    Seq("_pi_".utf8, "tag1".utf8).forall(result(3).key.labelValues.contains) shouldEqual true
    Seq("_step_".utf8, "tag1".utf8).forall(result(0).key.labelValues.contains) shouldEqual true
